...
import com.urim.cloudpicturebackend.api.imagesearch.ImageSearchApiFacade;
...
import com.urim.cloudpicturebackend.model.vo.PictureTageCategory;
...
public class PictureController {
    ...
    public BaseResponse<Page<PictureVo>> listPictureVOByPage(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        if (spaceId == null) {
            // 公开图库
            // 普通用户默认只能看到审核通过的数据
            pictureQueryRequest.setReviewStatus(PictureRewiewStatusEnum.PASS.getValue());
...
//            }
    ...
    public BaseResponse<Page<PictureVo>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        // 多级缓存 先查本地缓存 如果本地缓存有 则返回 如果没有则查询redis
        // 1. 先从本地缓存中查
        ...
        // 2. 本地缓存未命中 操作 redis 查看是否有key
        ValueOperations<String, String> opsForValue = stringRedisTemplate.opsForValue();
        cachedValue = opsForValue.get(cacheKey);
        if (cachedValue != null) {
            // 如果redis缓存命中， 更新本地缓存 返回缓存结果
        ...
        // 4, 更新缓存 查询完数据库后，将结果缓存到redis\本地缓存中
        String cacheValue = JSONUtil.toJsonStr(pictureVOPage);
        // 设置过期时间 5-10min 为了预防缓存雪崩
    ...
    public BaseResponse<PictureTageCategory> listPictureTagCategory() {
        PictureTageCategory pictureTagCategory = new PictureTageCategory();
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Integer> doPictureByBatch(@RequestBody PictureUploadByBatchRequest pictureUploadByBatchRequest,
                                                  HttpServletRequest request) {
    ...
    public BaseResponse<List<ImageSearchResult>> searchPictureByPicture(@RequestBody SearchPictureByPictureRequest searchPictureByPictureRequest) {
        ...
        List<ImageSearchResult> images = ImageSearchApiFacade.getImages(picture.getUrl());
